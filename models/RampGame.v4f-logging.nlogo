...
to act-on-changes
  if freeze? [wait 5 set freeze? false]      ; freezes the game for 10 sec. 
   ...
   if score-last-run > upper-break [  ; if the user did well
     ...
     if next-step > n-steps [   ; if the user has completed all the steps in this level
       if level < max-level [           ; if this isn't the last level
         ...
         stop]
       if level >= max-level [          ; if the student is on the last level, keep at the highest step.  ; 
...
;    set air-friction .2
  ...
  if level = 4 [
    set instructions ""
    if step = 1 [set instructions "Challenge 4: Make a heavier car stop in the center of the red area. This car is twice the mass of the last car."]
;    set instructions word instructions "\nYou will find it helpful to change the x-axis of the graph to ramp-height."
  ...
  if level = 5 [
    set instructions ""
    if level = 1 [set instructions "Challenge 5: Now made the car stop in the center of the red area by changing the friction. You cannot change the car's starting position."
...
;    set air-friction .2
    ...
    set target-radius-max .5 ; the distance between the center and edge of the target for step 1
    set target-radius-min .2 ; the distance for the highest step in this level
    set target-max item (step - 1) [3.2 2.1 3.9 .6 ]  ; move the target to predetermined places
...
;    set number-shown-already number-shown-already mod 4
    ...
    if number-shown-already = 2 [
      set m "Hint: The graph can help you find the best place to start the car. "]    
    if number-shown-already = 3 [
      set m "Hint: Look carefully at the graph that shows starting height and distance traveled. "
...
;    set number-shown-already number-shown-already mod 4
    ...
    if number-shown-already = 3 [    
      set m "Hint: use the new graph of distance against starting height to predict starting positions at this challenge." ] ]       
